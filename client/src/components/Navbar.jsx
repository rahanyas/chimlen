...
const menuVariants = {
  ...
  visible: { y: 70, opacity: 1, transition: { duration: 0.3 } },
};
  ...
  return (
    <motion.nav initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-black w-full text-white flex items-center justify-between px-6 p-5 fixed top-0 left-0 right-0 z-10">
      ...
      <div className="hidden md:flex gap-6">
        <motion.button variants={btnVariants} whileHover="hover" whileTap="tap" className="rounded-full px-4 py-2">Login</motion.button>
        <motion.button variants={btnVariants} whileHover="hover" whileTap="tap" className="rounded-full px-4 py-2">Sign-in</motion.button>
      </div>
      ...
      {isMobile && (
        <motion.div variants={menuVariants} initial="hidden" animate="visible" exit="hidden" className="fixed top-0 left-0 w-full h-fit bg-gray-900 flex flex-col items-center justify-center gap-6 p-6 md:hidden">
          ...
          <motion.button whileHover="hover" whileTap="tap" variants={btnVariants} className="rounded-full px-4 py-2">Login</motion.button>
          <motion.button whileHover="hover" whileTap="tap" variants={btnVariants} className="rounded-full px-4 py-2">Sign-in</motion.button>
        </motion.div>
      )}
    </motion.nav>
  );
